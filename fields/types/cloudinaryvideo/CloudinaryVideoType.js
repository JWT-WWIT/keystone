...
cloudinaryvideo.prototype.validateInput = function(data) {//eslint-disable-line no-unused-vars
...
cloudinaryvideo.prototype.updateItem = function(item, data) {
	...
	_.each(['public_id', 'version', 'signature', 'format', 'resource_type', 'url',
			'width', 'height', 'secure_url', 'duration', 'rotation'], setValue);
};
...
cloudinaryvideo.prototype.getRequestHandler = function(item, req, paths, callback) {
	...
	return function() {
		...
		if (req.body && req.body[paths.select]) {
			...
			uploadOptions.eager = field.options.eager;
			uploadOptions.eager_async = field.options.eager_async;
			uploadOptions.notification_url = field.options.notification_url;
			uploadOptions.eager_notification_url = field.options.eager_notification_url;
