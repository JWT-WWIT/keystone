...
module.exports = Field.create({
	...
	getImageSource: function() {
      ...
      // Remove the video extension and add .jpg
			return this.props.value.secure_url.replace(/\.\w+$/, '.jpg');
		} else {
			return null;
		}
	},
	...
	renderImagePreview: function() {
		...
		if (url) {
			body = <a className='img-thumbnail video' href={this.getImageURL()}>{body}</a>;
		} else {
			body = <div className='img-thumbnail video'>{body}</div>;
		}
		...
		if (!this.hasLocal() && !this.state.removeExisting) {
			values = (
				<div className='image-values'>
					<div className='field-value'>{this.props.value.secure_url}</div>
		...
		if (this.hasLocal()) {
			return (
				<div className='upload-queued pull-left'>
					<div className='alert alert-success'>Video selected - save to upload</div>
				</div>
			);
	...
	renderClearButton: function() {
		if (this.state.removeExisting) {
			...
			if (this.hasLocal()) {
				...
				clearText = (this.props.autoCleanup ? 'Delete Video' : 'Remove Video');
			}
			return (
			...
			<div key={this.props.path + '_toolbar'} className='image-toolbar'>
				<div className='pull-left'>
					<button type='button' onClick={this.changeImage} className='btn btn-default btn-upload-image'>
						{this.hasImage() ? 'Change' : 'Upload'} Video
					</button>
	...
	renderImageSelect: function() {
		...
		var getOptions = function(input, callback) {
			$.get(Keystone.contextPath + '/admin/api/cloudinary/autocomplete', {
				dataType: 'json',
				data: {
					q: input
				},
